...
public class WhatsAppServer {
    ...
    static class ContactsHandler implements HttpHandler {
        public void handle(HttpExchange exchange) throws IOException {
            enableCORS(exchange);            if ("OPTIONS".equals(exchange.getRequestMethod())) {
                exchange.sendResponseHeaders(204, -1);
                return;
            }            if ("OPTIONS".equals(exchange.getRequestMethod())) {
                exchange.sendResponseHeaders(204, -1);
            ...
            if ("GET".equals(exchange.getRequestMethod())) {
                ...
                if (messages != null) {
                    ...
                    for (Message m : messages.values()) {
                        String isoTime = m.getMessageTime().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME);
                        json.append("{\"messageId\":\"").append(m.getMessageID())
                            ...
                            .append("\",\"messageTime\":\"").append(isoTime).append("\"}");
